...
import { CardRect } from "@/components/animations/AnimationCard";
...
export default function AnimationDetailPage() {
	...
	useEffect(() => {
		...
		const fetchAnimation = async () => {
			try {
				const res = await fetch(`${getApiUrl()}/api/public/animations/${slug}`, {
					cache: "no-store",
				});
	...
	// Transform animation data to match AnimationCard interface
	const transformAnimation = (animation: Animation) => ({
		...
		videoThumbnailUrl: animation.videoThumbnailUrl || animation.videoUrl || "",
	});

	const handleSelectAnimation = (animation: any, rect: CardRect) => {
				...
				<main className="min-h-screen bg-black text-white flex items-center justify-center px-4">
					<div className="max-w-2xl mx-auto text-center">
						...
						<div className="mb-12">
							...
							<p className="text-gray-400 text-lg">The animation you're looking for doesn't exist.</p>
						</div>
			...
			{animation && (
				<div className="relative h-screen w-full">
					...
					<div className="absolute inset-0">
						{animation.videoThumbnailUrl ? (
							<img src={animation.videoThumbnailUrl} alt={animation.title} className="w-full h-full object-cover" />
						) : animation.videoUrl ? (
							<video src={animation.videoUrl} autoPlay muted loop playsInline className="w-full h-full object-cover" />
						) : (
					...
					<div
						...
						onClick={() => {
							const rect = document.querySelector(".hero-animation-trigger")?.getBoundingClientRect();
							if (rect) {
								handleSelectAnimation(transformAnimation(animation), {
									top: rect.top,
									left: rect.left,
									width: rect.width,
									height: rect.height,
					...
					>
						<div className="hero-animation-trigger">
							{animation.client && (
								<p className="text-gray-400 text-sm uppercase tracking-widest mb-2">{animation.client}</p>
							)}
							<h1 className="text-white text-4xl md:text-6xl font-bold mb-4">{animation.title}</h1>
							{animation.shortDescription && (
								<p className="text-gray-300 text-lg max-w-2xl mb-6">{animation.shortDescription}</p>
							)}
