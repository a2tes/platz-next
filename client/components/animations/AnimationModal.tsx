...
export default function AnimationModal({ animation, cardRect, onClose }: AnimationModalProps) {
	...
	useEffect(() => {
		...
		if (!showVideo || !video || !animation) return;

		// Determine best video source: HLS > Optimized MP4 > Original
		const hlsUrl = animation.hlsUrl;
		const mp4Url = animation.optimizedVideoUrl || animation.videoUrl;
		...
		if (hlsUrl && Hls.isSupported()) {
			...
			hls.on(Hls.Events.ERROR, (_event, data) => {
				if (data.fatal) {
					console.warn("[AnimationModal] HLS error, falling back to MP4:", data);
					cleanup();
					// Fallback to MP4
					if (mp4Url) {
						video.src = mp4Url;
	...
	if (!mounted || !animation || !cardRect) return null;

	return createPortal(
		<>
			{/* Backdrop */}
			<motion.div
				key={`modal-backdrop-${animation.slug}`}
				initial={{ opacity: 0 }}
				animate={{ opacity: 1 }}
				exit={{ opacity: 0 }}
			...
			<motion.div
				key={`modal-content-${animation.slug}`}
				initial={{
					position: "fixed",
					top: cardRect.top,
					left: cardRect.left,
									...
									{animation.client && (
										<div className="hidden md:block">
											<div>
												...
												<p className="text-sm md:text-base">{animation.client}</p>
											</div>
										</div>
									)}
