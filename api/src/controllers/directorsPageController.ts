...
export class DirectorsPageController {
	...
	async getSelections(req: Request, res: Response, next: NextFunction) {
		try {
			const rows = await directorsPageService.getSelections();
	...
	async addSelection(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const result = await directorsPageService.addSelection(directorId, workId);

			await ActivityService.log({
				userId: req.user!.id,
				action: "publish",
				module: "directors-page",
				itemType: "directorsPageSelection",
				itemId: result.id,
				itemTitle: result.work?.title || "Work",
				description: `Work added to directors page`,
			});
	...
	async removeSelection(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const result = await directorsPageService.removeSelection(id);
			...
			await ActivityService.log({
				...
				itemId: id,
				itemTitle: "Work",
				description: `Work removed from directors page`,
			});
	...
	async reorder(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const result = await directorsPageService.reorderItems(itemIds);
			res.json(apiResponse.success(result));
		} catch (err) {
			next(err);
		}
	...
	async updateVideoSource(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const result = await directorsPageService.updateVideoSource(selectionId, videoSource, clipJobId);
			res.json(apiResponse.success(this.serializeRow(result)));
		} catch (err) {
			next(err);
		}
	...
	async processClip(req: Request, res: Response, next: NextFunction) {
		try {
			...
			const selection = await directorsPageService.getSelection(selectionId);
			...
			await directorsPageService.updateVideoSource(selectionId, "clip", jobResult.clipJobId);
...
export const directorsPageController = new DirectorsPageController();
