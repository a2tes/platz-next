...
import { agenciesService, Agency, CreateAgencyData, UpdateAgencyData } from "@/services/agenciesService";
...
export interface AgenciesModalProps {
	...
	agency?: Agency | null;
	onSaved?: (saved: Agency | void) => void;
}

export function AgenciesModal({ open, onOpenChange, agency, onSaved }: AgenciesModalProps) {
	...
	const {
		...
		defaultValues: {
			name: agency?.name || "",
			slug: agency?.slug || "",
			status: agency?.status || "PUBLISHED",
		},
	});
	...
	const createMutation = useMutation({
		mutationFn: (data: CreateAgencyData) => agenciesService.createAgency(data),
		onSuccess: (saved) => {
	...
	const updateMutation = useMutation({
		mutationFn: (data: UpdateAgencyData) => agenciesService.updateAgency(agency!.id, data),
		onSuccess: (saved) => {
			toast.success("Agency updated");
			queryClient.invalidateQueries({ queryKey: ["agencies"] });
			queryClient.invalidateQueries({ queryKey: ["agencies-counts"] });
			if (agency) queryClient.invalidateQueries({ queryKey: ["agency", agency.id] });
			onSaved?.(saved);
			onOpenChange(false);
		},
		onError: (error: unknown) => {
	...
	const onSubmit = async (data: AgencyFormData) => {
		const payload = {
			...
			status: data.status,
		} as CreateAgencyData & UpdateAgencyData;

		if (isEditing) {
			await updateMutation.mutateAsync(payload);
		} else {
			await createMutation.mutateAsync(payload as CreateAgencyData);
		}
	};
			...
			<DialogContent className="sm:max-w-md p-0">
				<DialogHeader className="px-6 py-4 border-b">
					<div className="flex items-center justify-between">
						<DialogTitle>{isEditing ? "Edit Agency" : "New Agency"}</DialogTitle>
				...
				<form onSubmit={handleSubmit(onSubmit, onError)} className="space-y-6 p-6">
					<div className="space-y-2">
						...
						<Input id="name" {...register("name")} placeholder="Agency name" autoFocus />
