...
import { Revision, Work } from "@/services/worksService";
...
function formatFieldValue(
	...
	if (typeof value === "number") {
		if (fieldName === "videoFileId") {
			...
			return work?.videoFile?.originalName || String(value);
		}
		if (fieldName === "previewImageId") {
			...
			return work?.previewImage?.originalName || String(value);
		}
	}
	...
	if (typeof value === "object") return "—"; // Fallback for objects
	return String(value);
}
							...
							{visibleRevisions.map((revision) => (
								<button
									...
									onClick={() => {
										...
										// For Initial State (V0), always use list view, no comparison
										...
										{typeof selectedRevision.payload === "object" &&
											Object.entries(selectedRevision.payload)
												.filter(([key, value]) => {
													...
													// Check if formatted value would be "—"
