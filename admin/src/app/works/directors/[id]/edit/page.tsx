...
export default function EditDirectorPage() {
	...
	const directorId = parseInt(params.id as string, 10);
	...
	const {
		...
		queryFn: () => WorksService.getDirector(directorId),
		enabled: !isNaN(directorId),
	});

	const updateTitleMutation = useMutation({
		mutationFn: (title: string) => WorksService.updateDirector(directorId, { title }),
		onSuccess: () => {
				...
				<div className="flex-1">
					<div className="mb-6">
						{isLoading ? (
							...
							<>
								...
								{director && (
									<a
										href={`${publicUrl}/directors/${director.slug}`}
										target="_blank"
									...
									>
										<span className="group-hover:underline">
											{publicUrl}/directors/{director.slug}
										</span>
								...
								<div className="flex flex-col sm:flex-row justify-between sm:items-center">
									<div className="flex flex-col">
										<div className="flex items-center gap-3">
											<h1 className="text-3xl font-bold tracking-tight">{director?.title || "Edit Director"}</h1>
											{director && (
									...
									<Button variant="outline" onClick={() => router.push("/works/directors")} className="mt-4 sm:mt-0">
					...
					{isError && (
						<div className="text-center py-12">
							<p className="text-red-600">Error loading director</p>
						</div>
					)}
