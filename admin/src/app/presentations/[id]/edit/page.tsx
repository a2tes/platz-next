...
export default function EditPresentationPage() {
	...
	const presentationId = parseInt(params.id as string, 10);
	...
	const {
		...
		queryFn: () => PresentationService.getById(presentationId),
		enabled: !isNaN(presentationId),
	});

	const updateTitleMutation = useMutation({
		mutationFn: (title: string) => PresentationService.update(presentationId, { title }),
		onSuccess: () => {
				...
				<div className="flex-1">
					<div className="mb-6">
						{isLoading ? (
							...
							<>
								...
								{presentation && (
									<a
										href={`${publicUrl}/presentation/${presentation.token}`}
										target="_blank"
									...
									>
										<span className="group-hover:underline">
											{publicUrl}/presentation/{presentation.token}
										</span>
							...
							<>
								<div className="flex flex-col sm:flex-row justify-between sm:items-center">
									<div className="flex flex-col">
										<div className="flex items-center gap-3">
											<h1 className="text-3xl font-bold tracking-tight">
												{presentation?.title || "Edit Presentation"}
											</h1>
											{presentation && (
									...
									<Button variant="outline" onClick={() => router.push("/presentations")} className="mt-4 sm:mt-0">
					...
					{isError && (
						<div className="text-center py-12">
							<p className="text-red-600">Error loading presentation</p>
						</div>
					)}

					{presentation && <PresentationForm key={presentation.updatedAt} initialData={presentation} />}
				</div>
			</AdminLayout>
		</ProtectedRoute>
	);
