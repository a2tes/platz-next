...
function ClientsTrashPageContent() {
	const router = useRouter();
	const queryClient = useQueryClient();
	const [search, setSearch] = React.useState("");
	const [page, setPage] = React.useState(1);
	const [itemToRestore, setItemToRestore] = React.useState<Client | null>(null);
	const [itemToPurge, setItemToPurge] = React.useState<Client | null>(null);

	const { data, isLoading, error } = useQuery({
		queryKey: ["trashed-clients", { page, search }],
		queryFn: () => clientsService.getTrashedClients({ page, limit: 20, search }),
	});

	const restoreMutation = useMutation({
		mutationFn: (id: number) => clientsService.restoreClient(id),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["trashed-clients"] });
	...
	const purgeMutation = useMutation({
		mutationFn: (id: number) => clientsService.purgeClient(id),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ["trashed-clients"] });
			queryClient.invalidateQueries({ queryKey: ["clients-counts"] });
			setItemToPurge(null);
		...
		<AdminLayout>
			<div className="container max-w-6xl py-8">
				...
				<div className="flex items-center justify-between mb-8">
					<div>
						<h1 className="text-3xl font-bold">Trashed Clients</h1>
						<p className="text-muted-foreground mt-1">Restore or permanently delete trashed clients</p>
					</div>
					<Button variant="outline" onClick={() => router.push("/entities/clients")}>
						Back to Clients
					</Button>
				</div>
				...
				{isLoading ? (
					...
					<div className="flex flex-col items-center justify-center py-12 text-destructive">
						...
						<p>Failed to load trashed clients</p>
					</div>
				) : items.length === 0 ? (
					<div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
						<IconTrashOff className="h-12 w-12 mb-4" />
						<p className="text-lg font-medium">Trash is empty</p>
						<p className="text-sm">Deleted clients will appear here</p>
					</div>
				) : (
			...
			<AlertDialog open={!!itemToRestore} onOpenChange={() => setItemToRestore(null)}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Are you sure you want to restore "{itemToRestore?.name}"? It will be moved back to your clients list.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
